...
anon_create()
{
        mmobj_t *the_mmobj = (mmobj_t*)slab_obj_alloc(anon_allocator);
        mmobj_init(the_mmobj, &anon_mmobj_ops);
        the_mmobj -> mmo_refcount = 1;
        dbg(DBG_PRINT, "(GRADING3D)\n");
...
anon_put(mmobj_t *o)
{
        ...
        if(o -> mmo_refcount - 1 == o -> mmo_nrespages){
                pframe_t *the_frame;
                list_iterate_begin(&(o -> mmo_respages), the_frame, pframe_t, pf_olink){
			
                        pframe_free(the_frame);
                        dbg(DBG_PRINT, "(GRADING3D)\n");
