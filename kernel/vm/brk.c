...
do_brk(void *addr, void **ret)
{
        ...
        uint32_t curr_pn = ADDR_TO_PN((uint32_t)(curproc -> p_brk) - 1);
        uint32_t new_pn = ADDR_TO_PN((uint32_t)(addr) - 1);
        vmmap_t *the_map = curproc -> p_vmmap;
        if(new_pn < curr_pn){
                vmmap_remove(the_map, new_pn + 1, curr_pn - new_pn);
        }
        else if(new_pn > curr_pn){
                if(!vmmap_is_range_empty(the_map, curr_pn + 1, new_pn - curr_pn)){
                        return -ENOMEM;
                }
		vmarea_t *the_area = vmmap_lookup(the_map, curr_pn);
                the_area -> vma_end = new_pn + 1;
        }
        curproc -> p_brk = addr;
        *ret = addr;
