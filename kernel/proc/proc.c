...
proc_create(char *name)
{
        ...
        // now curproc means the parent process of this new process
        proc_t *process = (proc_t*) slab_obj_alloc(proc_allocator); // allocate a new process


        process->p_pid = _proc_getid(); // get a pid from getid()
        ...
        int length = MIN(strlen(name),PROC_NAME_LEN);
        strncpy(process->p_comm,name,length);
        process->p_comm[length] = '\0';
        ...
        process->p_pproc = curproc; // NULL if process doesn't have a parent

        process->p_status = 0;
        process->p_state = PROC_RUNNING; // new process initialized as running
        ...
        if(curproc!=NULL){
                // have a parent process(not idle), insert p_child_link to curproc->p_children
                dbg(DBG_PRINT, "(GRADING1C)\n");
        ...
        // other variables are used in kernel 2&3
        dbg(DBG_PRINT, "(GRADING1C)\n");
        return process;
}
...
proc_kill_all()
{
        ...
        list_iterate_begin(proc_list(),p,proc_t,p_list_link){
                // kill all process except whose pid = PID_IDLE OR whose parent process's PID = PID_IDLE
...
do_waitpid(pid_t pid, int options, int *status)
{
	...
	if(pid==-1){
		while(1){
				// block if all children of current process is running
			list_iterate_begin(&curproc->p_children,p,proc_t,p_child_link){
				if(p->p_state==PROC_DEAD){
					dbg(DBG_PRINT, "(GRADING1C)\n");
					// dipose it and return its exit status
					if(status!=NULL){
						*status = p->p_status;
					}
					...
					list_iterate_begin(&p->p_threads,kth,kthread_t,kt_plink){
						kthread_destroy(kth);
					}list_iterate_end();
			...
			// all children are running, block here
		...
		while(1){
			...
			if(p==NULL || p->p_pproc!=curproc){
				// target does not exist or is not the children of the parent
				dbg(DBG_PRINT, "(GRADING1C)\n"); 
				return -ECHILD;
			}
			...
			// list_iterate_begin(&curproc->p_children,flag,proc_t,p_child_link){
			// 	if(flag->p_state==PROC_DEAD){
			// 		dbg(DBG_PRINT, "(GRADING1C)\n");
			// 		list_iterate_begin(&flag->p_threads,kth,kthread_t,kt_plink){
			// 				kthread_destroy(kth);
			// 		}list_iterate_end();
			// 		list_remove(&flag->p_child_link);
			// 		list_remove(&flag->p_list_link);
