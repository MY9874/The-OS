...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        if((vmarea->vma_prot & perm) == perm){
                dbg(DBG_PRINT, "(GRADING3B)\n");

                return 1;
        }return 0;
        // dbg(DBG_PRINT, "addr_perm_break4\n");
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        uint32_t curraddr;
        for(int i = 0; i <= (int)len; i+= PAGE_SIZE){
                dbg(DBG_PRINT, "(GRADING3B)\n");
                curraddr = (uint32_t)avaddr + i;
                if(addr_perm(p, (const void*)curraddr, perm) == 0){
                        dbg(DBG_PRINT, "(GRADING3B)\n");
                        return 0;
                }
        }
