...
sys_read(read_args_t *arg)
{
        ...
        read_args_t kt_args;
        //int copy_res = 
        copy_from_user(&kt_args, arg, sizeof(read_args_t));
        // if(copy_res < 0){
        //         dbg(DBG_PRINT, "sys_read_break1\n");

        //         curthr->kt_errno = -copy_res;
        //         return -1;
        // }

        void *buffer = page_alloc();
        int read_res = do_read(kt_args.fd, buffer, kt_args.nbytes);
        if(read_res < 0){
                dbg(DBG_PRINT, "(GRADING3B)\n");
                curthr->kt_errno = -read_res;
                page_free(buffer);
                return -1;
        }

        //copy_res = 
        copy_to_user(arg->buf, buffer, read_res);
        // if(copy_res < 0){
        //         dbg(DBG_PRINT, "sys_read_break3\n");
        //         curthr->kt_errno = -copy_res;
        //         return -1;
        // }
...
sys_write(write_args_t *arg)
{
        ...
        write_args_t kt_args;
        //int copy_res = 
        copy_from_user(&kt_args, arg, sizeof(write_args_t));
        // if(copy_res < 0){
        //         dbg(DBG_PRINT, "sys_write_break1\n");
        //         curthr->kt_errno = -copy_res;
        //         return -1;
        // }
        ...
        //copy_res = 
        copy_from_user(buffer, arg->buf, arg->nbytes);
        // if(copy_res < 0){
        //         dbg(DBG_PRINT, "sys_write_break2\n");
        //         curthr->kt_errno = -copy_res;
        //         return -1;
        // }

        int write_res = do_write(kt_args.fd, buffer, kt_args.nbytes);
        if(write_res < 0){
                dbg(DBG_PRINT, "(GRADING3B)\n");
                curthr->kt_errno = -write_res;
                page_free(buffer);
                return -1;
        }       
...
sys_getdents(getdents_args_t *arg)
{
        ...
        getdents_args_t kdent_args;
        //int copy_res = 
        copy_from_user(&kdent_args, arg, sizeof(getdents_args_t));
        // if(copy_res < 0){
        //         dbg(DBG_PRINT, "sys_getdents_break1\n");
        //         curthr->kt_errno = -copy_res;
        //         return -1;
        // }
        ...
        dirent_t d;
        int total_bytes = 0;
        int num_dirent = arg->count/sizeof(dirent_t);
        for(int i = 0; i < num_dirent; i++){
                int get_dent_res = do_getdent(kdent_args.fd, &d);
                if(get_dent_res < 0){
                        dbg(DBG_PRINT, "(GRADING3B)\n");
                        curthr->kt_errno = -get_dent_res;
                        return -1;
                }
                else if(get_dent_res == 0){//reach end of file
                        dbg(DBG_PRINT, "(GRADING3B)\n");
                        break;
                }

                copy_to_user(arg->dirp+i, &d, get_dent_res);
                total_bytes+= get_dent_res;
                dbg(DBG_PRINT, "(GRADING3B)\n");
        }
